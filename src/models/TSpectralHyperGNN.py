...
from .tspectralconv import TSpectralConvolution
from .mlp import MLP
...
class TSpectralHyperGNN(Module):
    def __init__(self, args):
        ...
        self.dict_dim = dict(
            zip([0, 1, 2], ["i,j->ij", "ij,k->ijk", "ijk,p->ijkp"])
        )  # used to compute outer product using einsum
    ...
    def forward(self, A, X):
        ...
        num_slices = int(
            torch.prod(torch.tensor(list(X.shape[2:])))
        )  # n3 x n4 x ... x np
    ...
    def HypergraphSignal_torch(self, S):
        ...
        d = S.shape[1]
        return torch.cat(
            [one_dim_hypergraphSignal(S[:, i], self.M) for i in range(d)], dim=1
        )  # concatenate along the second order
