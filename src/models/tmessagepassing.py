...
class TMessagePassing(nn.Module):
    ...
    def __init__(self, features, structure, M: int, device: torch.device):
        """
    ...
    def forward(self, target_nodes):
        ...
        """
        neigh_feats = torch.stack(
            [self.aggregate_for_one_node(target_node) for target_node in target_nodes],
            dim=0,
        )
        return neigh_feats

    def aggregate_for_one_node(self, target_node):
        target_node = target_node.to(self.device)
    ...
    def aggregate_with_M(self, edge, target_node):
        ...
        idx = torch.tensor(tmp_edge, dtype=torch.long, device=self.device)
    ...
    def aggregate_with_c(self, edge, target_node):
        ...
        all_comb = [
            list(t) for t in itertools.combinations_with_replacement(edge, self.M)
        ]  # all possible combs to fill in length-M list
        val_comb = list(
            filter(lambda comb: set(comb) == set(edge), all_comb)
        )  # each node must appear at least once

        tmp_comb = copy.deepcopy(val_comb)
        for comb in tmp_comb:
            comb.remove(target_node)

        num_perms = torch.Tensor(
            [len(list(set(itertools.permutations(comb)))) for comb in tmp_comb]
        )
        # cross multiply features
        high_order_signal = torch.stack(
            [
                torch.prod(
                    self.features(
                        torch.tensor(comb, dtype=torch.long, device=self.device)
                    ),
                    dim=0,
                )
                for comb in tmp_comb
...
class Encoder(nn.Module):
    ...
    def __init__(
        self, features, input_dim, output_dim, args, aggregator, base_model=None
    ):
        """
        ...
        self.W = Parameter(
            torch.FloatTensor(
                self.input_dim if self.combine == "sum" else 2 * self.input_dim,
                self.output_dim,
            )
        )
    ...
    def forward_with_features(self, X: torch.Tensor, neig_dict, nodes: torch.Tensor):
        # update aggregator to use dynamic features/neighbors
        self.aggregator.features = lambda idx: X[idx]
        self.aggregator.structure = neig_dict
        # standard TMPHN forward over nodes
    ...
    def forward(self, nodes):
        ...
        n = nodes.to(self.device).long()
        self_feats = self.aggregator.features(
            n
        )  # NOTE: review this used to be self.features
        if self.combine == "concat":
            combined = torch.cat(
                [self_feats, neigh_feats], dim=1
