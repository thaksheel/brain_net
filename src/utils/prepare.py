...
from .TensorRep import HyperRepresentation
...
def read_data(dataset_directory, data_type=None, dataset=None):
    # dir = os.path.join(dataset_directory, data_type, dataset)

    # load hypergraph
    with open(dataset_directory + "hypergraph.pickle", "rb") as handle:
        hypergraph = pickle.load(handle)
    # load feature
    with open(dataset_directory + "feature.pickle", "rb") as handle:
        feature = pickle.load(handle)
    # load labels
    with open(dataset_directory + "labels.pickle", "rb") as handle:
...
def initialize(H, X, Y, args, seed_split: int, device: torch.device):
    """
    ...
    if args.method == "T-Spectral" or args.method == "T-Spatial":
        ...
        if args.method == "T-Spectral":
            model = TSpectralHyperGNN(args)
        else:
            model = TSpatialHyperGNN(args)
        model.to(device)
    elif args.method == "T-MPHN":
        all_nodes = list(np.arange(X.shape[0]))
        neig_dict = NeighborFinder(H, args.M).neig_for_targets(all_nodes)
        data = {"hypergraph": neig_dict, "X": X, "Y": Y.to(device)}
        model = TMPHN(
            X, neig_dict, args
        )  # the TMPHN model is loaded to the device in args automatically
    # optimizer
    ...
    split_idx = rand_train_test_idx(
        seed=seed_split,
        label=Y,
        train_prop=args.train_ratio,
        valid_prop=args.valid_ratio,
    )
